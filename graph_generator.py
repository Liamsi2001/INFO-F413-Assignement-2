...
def generate_and_save_graphs():
    ...
    graph_types = [
        ...
        ("bipartite", lambda n: nx.complete_bipartite_graph(2*n//5, 3*n//5)),
...
def generate_and_save_exotic_graphs():
    ...
    exotic_graph_types = [
        ...
        ("watts_strogatz", lambda n: ensure_connected(nx.watts_strogatz_graph(n, n//4, 0.25, seed=SEED))),
        ("barabasi_albert", lambda n: ensure_connected(nx.barabasi_albert_graph(n, max(1, n//5), seed=SEED))),
        ("Power-Law Cluster", lambda n: generate_powerlaw_cluster(n, max(1, n//10), 0.25, seed=SEED)),
    ]
